...
- **External editor** (`external_editor.rs`): "Open in <editor>" (`win.open-in-editor`, Ctrl+Shift+O), with **two different implementations depending on the sandbox**.
  ...
  - *Under Flatpak*: the feature is **hidden entirely** — `setup_actions` does not register `win.open-in-editor`, `build_primary_menu` takes `editor_label: Option<&str>` and omits the entry, and the preferences group is not added. `detect_installed` returns empty and `launch` errors out as a backstop. `GP_SIMULATE_FLATPAK=1` forces this branch on a normal desktop, which is the only way to exercise it while developing.
  - **Two dead ends, both already tried — do not repeat them.** (1) `flatpak-spawn --host` needs `--talk-name=org.freedesktop.Flatpak`; Flathub's linter rejects it as `finish-args-flatpak-spawn-access` and v1.3.0's Flathub build failed on exactly that check. An exception must be granted by hand in `flathub-infra/flatpak-builder-lint`. (2) The XDG desktop portal (`gtk::FileLauncher` with `always_ask`) is **not** a workaround: its chooser only lists applications registering as `inode/directory` handlers, which editors generally do not — Zed ships that MimeType line commented out deliberately, noting it made Zed the default file browser. Verified on a real desktop: the chooser offered Files, Disk Usage Analyzer and Gitpulsar itself, no editor.
  - Preferences preserves the stored command when sandboxed instead of overwriting it with "Not configured" — the same config file may be shared with a non-Flatpak install.
