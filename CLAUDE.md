...
- **CLI open**: `HANDLES_OPEN` makes GApplication emit `open` **instead of** `activate` when paths are passed, so both signals must build the window — hence `present_window` in `app.rs`, with the display setup behind a `Once`. Connecting only `activate` (as the app did until v1.3.3) means `gitpulsar-gtk /path/to/repo` prints a GIO warning and opens nothing. The window's deferred "restore last workspace" also has to yield to it: that runs from `idle_add_local_once` and would otherwise overwrite the path a moment after it was opened, so it returns early when `workspace_root` is already set.
