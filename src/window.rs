...
use crate::utils::workspace::{self, WorkspaceEntry};

use crate::utils::config::{AppConfig, DateFormat};
use crate::utils::undo::{UndoStack, UndoableOp};
...
use crate::widgets::branches_tags_panel;
use crate::widgets::changed_file_object;
use crate::widgets::changes_view;
use crate::widgets::blame_view;
...
use crate::widgets::gitignore_editor;
use crate::widgets::header_chrome;
...
mod imp {
    ...
    impl Default for GitpanelWindow {
        fn default() -> Self {
            Self {
    ...
    impl ObjectSubclass for GitpanelWindow {
        const NAME: &'static str = "GitpanelWindow";
        type Type = super::GitpanelWindow;
        type ParentType = adw::ApplicationWindow;
    }

    impl ObjectImpl for GitpanelWindow {}
    impl WidgetImpl for GitpanelWindow {}
    impl WindowImpl for GitpanelWindow {}
    impl ApplicationWindowImpl for GitpanelWindow {}
    impl AdwApplicationWindowImpl for GitpanelWindow {}
}

glib::wrapper! {
    pub struct GitpanelWindow(ObjectSubclass<imp::GitpanelWindow>)
...
impl GitpanelWindow {
    pub fn new(app: &adw::Application) -> Self {
        let width = std::env::var("GP_WIDTH").ok().and_then(|s| s.parse::<i32>().ok()).unwrap_or(1200);
        let height = std::env::var("GP_HEIGHT").ok().and_then(|s| s.parse::<i32>().ok()).unwrap_or(800);
        let window: Self = glib::Object::builder()
            .property("application", app)
            .property("title", "Gitpanel")
        ...
        // workspace opened in the meantime — `gitpanel-gtk /path/to/repo`
    ...
    fn setup_ui(&self) {
        ...
        // Ahead/behind labels still get updated for header use, but no longer rendered
        // in the bottom bar — the repo sidebar dots already convey the same info.
        imp.ahead_label.set_visible(false);
        imp.behind_label.set_visible(false);
        ...
        imp.search_entry.connect_search_changed(move |entry| {
            ...
            if let Some(filter) = win.imp().commit_filter.borrow().clone() {
                crate::widgets::commit_list::set_commit_filter(&filter, &query);
            }
        });
        ...
        {
            use crate::widgets::commit_list as commit_list_v;
        ...
        *imp.worktrees_list.borrow_mut() = Some(wt_list);
        ...
        // Compact (<1080sp): collapse right sidebar to overlay
        let bp_compact = adw::Breakpoint::new(adw::BreakpointCondition::new_length(
    ...
    fn rebuild_hamburger_menu(&self) {
        ...
        // particular editor.
        let editor_label = if crate::utils::external_editor::in_flatpak() {
            "Open With…".to_string()
        } else {
            match config.external_editor.as_deref() {
                Some(cmd) => format!("Open in {}", crate::utils::external_editor::label_for_command(cmd)),
    ...
    fn setup_actions(&self) {
        ...
        // Export graph action
        let export_graph_action = gio::SimpleAction::new("export-graph", None);
        let window = self.clone();
        export_graph_action.connect_activate(move |_, _| {
            window.show_export_graph_dialog();
        });
        self.add_action(&export_graph_action);

        // Bisect actions
        let bisect_start_action = gio::SimpleAction::new("bisect-start", None);
        let window = self.clone();
        bisect_start_action.connect_activate(move |_, _| {
            window.show_bisect_start_dialog();
        });
        self.add_action(&bisect_start_action);
        ...
        about_action.connect_activate(move |_, _| {
            let about = adw::AboutDialog::builder()
                .application_icon("io.github.liangzhaoyuan12.gitpanel")
                .application_name("Gitpanel")
                .version(env!("CARGO_PKG_VERSION"))
                .developer_name("liangzhaoyuan12")
                .comments("由 liangzhaoyuan12 维护")
                .license_type(gtk::License::Gpl30)
                .website("https://github.com/liangzhaoyuan12/gitpanel")
                .build();
            about.add_link("Gitee 网站", "https://gitee.com/liangzhaoyuan12/gitpanel");
            about.add_link("cnb 网站", "https://cnb.cool/liangzhaoyuan12/gitpanel");
            about.add_acknowledgement_section(
                Some("鸣谢"),
                &[
                    "GitPulsar by Ilshat Ishdavletov",
                    "https://gitlab.com/ilshat-apps/gitpulsar",
                ],
            );
    ...
    fn load_repo_data(&self, repo: &GitRepo) {
        ...
        *imp.selected_commit_id.borrow_mut() = None;
        ...
        if let Some(filter) = imp.commit_filter.borrow().clone() {
            crate::widgets::commit_list::set_commit_filter(&filter, "");
        }
        ...
        glib::spawn_future_local(async move {
            if let Ok(data) = rx.recv().await {
                ...
                *imp.cached_staged_diffs.borrow_mut() = staged;
                ...
                win.update_conflict_banner(data.has_conflicts, data.is_merging, data.is_rebasing, data.is_bisecting);
    ...
    fn spawn_signature_batch(&self, ids: Vec<String>) {
        ...
        glib::spawn_future_local(async move {
            ...
            for id in crate::widgets::commit_list::apply_signature_flags(&store, &flags) {
    ...
    fn on_commit_activated_by_id(&self, commit_id: &str) {
        ...
        let n = store.n_items();
        let mut found: Option<(u32, crate::widgets::commit_object::CommitObject)> = None;
        for i in 0..n {
            if let Some(obj) = store
                .item(i)
                .and_then(|o| o.downcast::<crate::widgets::commit_object::CommitObject>().ok())
            {
    ...
    fn open_commit_file_diff(&self, commit_id: &str, path: &str) {
        ...
        for i in 0..store.n_items() {
            ...
            let refs = crate::widgets::commit_diff_dialog::build_commit_diff_dialog(
                &obj.short_id(),
                &obj.summary(),
                &files[..shown],
                path,
    ...
    fn rebind_commit_row(&self, commit_id: &str, obj: &crate::widgets::commit_object::CommitObject) {
        let imp = self.imp();
        let Some(list_view) = imp.commit_list_view.borrow().clone() else { return };
        let Some(outer) = crate::widgets::commit_list::find_row_outer(&list_view, commit_id) else {
            return;
        };
    ...
    fn on_commit_clicked(&self) {
        ...
        let allow_empty = imp.allow_empty_check.is_active();

        let repo_ref = imp.repo.borrow();
        if let Some(ref repo) = *repo_ref {
            if !is_amend && !allow_empty {
                let has_staged = repo.status(false)
                ...
                if !has_staged {
                    ...
                    let dialog = adw::AlertDialog::new(
                        ...
                        Some("No files are staged. Stage changes first, or tick \"Allow empty\" to commit anyway."),
                    );
                    dialog.add_response("ok", "OK");
                    dialog.present(Some(self));
                    return;
            ...
            match result {
                Ok(oid) => {
                    ...
                    imp.allow_empty_check.set_active(false);
                    drop(repo_ref);
                    // Reload repo data
    ...
    fn refresh_active_repo_indicator(&self, path_str: &str) {
        ...
        let new_indicator = crate::utils::workspace::RepoIndicator {
    ...
    /// Route a click on a per-file row button to the right handler.
    fn dispatch_row_button(&self, path: &str, name: &str) {
        match name {
            "stage-file" => self.stage_file(path),
            "unstage-file" => self.unstage_file(path),
            "discard-file" => self.discard_file_with_confirm(path),
            "blame-file" => self.show_blame(path),
            "history-file" => self.show_file_history(path),
            _ => {}
        }
    }
    ...
    fn populate_branches_tags_data(
        &self,
        branches: &[crate::model::BranchInfo],
        tags: &[crate::model::TagInfo],
    ) {
        let imp = self.imp();
    ...
    fn update_conflict_banner(&self, has_conflicts: bool, is_merging: bool, is_rebasing: bool, is_bisecting: bool) {
        ...
        if !has_conflicts && !is_merging && !is_rebasing && !is_bisecting {
            return;
        }
        ...
        let msg = if has_conflicts {
            "Conflicts detected — resolve files and mark as resolved".to_string()
        } else if is_rebasing {
            "Rebase in progress".to_string()
        } else if is_bisecting {
            if bisect_status.is_empty() {
                "Bisecting".to_string()
            } else {
                format!("Bisecting — {}", bisect_status)
            }
        } else {
        ...
        if is_bisecting {
            let good_btn = gtk::Button::builder()
                ...
                .build();
            let win = self.clone();
            good_btn.connect_clicked(move |_| {
                win.run_git_op("Bisect Good", |path| {
                    run_git_cmd(path, &["bisect", "good"])
            ...
            let bad_btn = gtk::Button::builder()
                ...
                .build();
            let win = self.clone();
            bad_btn.connect_clicked(move |_| {
                win.run_git_op("Bisect Bad", |path| {
                    run_git_cmd(path, &["bisect", "bad"])
            ...
            skip_btn.connect_clicked(move |_| {
                win.run_git_op("Bisect Skip", |path| {
                    run_git_cmd(path, &["bisect", "skip"])
            ...
            reset_btn.connect_clicked(move |_| {
                win.run_git_op("Bisect Reset", |path| {
                    run_git_cmd(path, &["bisect", "reset"])
    ...
    fn bisect_status_line(&self) -> String {
        let Some(path) = self.repo_path_string() else { return String::new() };
        // First try the short summary git provides
        if let Ok(out) = run_git_cmd(&path, &["bisect", "view", "--format=%h %s"]) {
            if let Some(first) = out.lines().next() {
                return first.to_string();
            }
        }
        if let Ok(out) = run_git_cmd(&path, &["log", "-1", "--format=%h %s"]) {
            return out.trim().to_string();
        }
        String::new()
    }

    fn show_bisect_start_dialog(&self) {
        if self.repo_path_string().is_none() {
        ...
        let bad_entry = gtk::Entry::builder()
            .text("HEAD")
            .build();
        ...
        let good_entry = gtk::Entry::builder()
            .placeholder_text("commit SHA or tag")
            .build();
        body.append(&good_entry);
        ...
        start_btn.connect_clicked(move |_| {
            let bad = bad_entry.text().to_string();
            let good = good_entry.text().to_string();
            if good.trim().is_empty() {
                win.show_error_dialog("Bisect", "Specify a good (older, working) ref.");
                return;
            }
            dlg.close();
            let bad = if bad.trim().is_empty() { "HEAD".to_string() } else { bad };
            win.run_git_op("Bisect Start", move |path| {
                run_git_cmd(path, &["bisect", "start"])?;
                run_git_cmd(path, &["bisect", "bad", &bad])?;
                let out = run_git_cmd(path, &["bisect", "good", &good])?;
                Ok(if out.trim().is_empty() { "Bisect started".to_string() } else { out.trim().to_string() })
            });
        });
    ...
    fn show_export_graph_dialog(&self) {
    ...
    fn open_conflict_editor(&self, path: &str, chunks: &[crate::utils::conflict::ConflictChunk]) {
    ...
    fn open_in_editor(&self) {
        ...
        if crate::utils::external_editor::in_flatpak() {
        ...
        match crate::utils::external_editor::launch(&command, std::path::Path::new(&repo_path)) {
            Ok(()) => {
                let label = crate::utils::external_editor::label_for_command(&command);
    ...
    fn trigger_background_refresh(&self) {
        ...
        std::thread::spawn(move || {
            ...
            let (status, status_hash, ahead, behind, unstaged_diffs, staged_diffs) =
                if let Some(ref p) = repo_path {
                    if let Ok(repo) = GitRepo::open(p) {
                        ...
                        if probe_hash == prev_status_hash {
                            ...
                            (None, probe_hash, a, b, Vec::new(), Vec::new())
                        } else {
                            // Change detected — full scan so UI gets every untracked file.
                            let st = repo.status(true).ok();
                            let ud = repo.diff_unstaged().unwrap_or_default();
                            let sd = repo.diff_staged().unwrap_or_default();
                            (st, probe_hash, a, b, ud, sd)
                        }
                    } else {
    ...
    fn setup_commit_context_menu(&self) {
        ...
        gesture.connect_released(move |_gesture, _, x, y| {
            ...
            // ListBox behaviour.
            let Some(store) = imp.commit_store.borrow().clone() else { return };
            let mut commit: Option<crate::widgets::commit_object::CommitObject> = None;
            ...
            // Export as Archive
            let export_archive_btn = gtk::Button::builder()
                .label("Export as Archive…")
                .css_classes(["flat"])
                .build();
            let sha_clone = sha.clone();
            let pp_ea = popover.clone();
            let w_ea = win.clone();
            export_archive_btn.connect_clicked(move |_| {
                pp_ea.popdown();
                w_ea.export_commit_archive(&sha_clone);
            });
            menu_box.append(&export_archive_btn);
            ...
            if let Ok(rows) = rx.recv().await {
                ...
                da.set_draw_func(move |_da, cr, _w, _h| {
                    for (i, row) in rows_for_draw.iter().enumerate() {
                        ...
                        crate::widgets::commit_graph::draw_graph_row_public(cr, row, 32.0, &rows_for_draw, i);
                        let _ = cr.restore();
                    }
                });
